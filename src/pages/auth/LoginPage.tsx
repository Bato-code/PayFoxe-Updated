...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (query.get('registered') === '1') {
      setSuccess('Account created! Please check your email for the verification link before logging in.')
    }
  }, [location])
    ...
    <div className="min-h-screen bg-gradient-purple flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        <div className="backdrop-blur-sm rounded-3xl p-8 shadow-glow-lg">
          ...
          {(success || error) && (
            <div className={`mb-4 p-3 rounded-lg text-sm text-center ${success ? 'bg-green-500/10 border border-green-500/50 text-green-400' : 'bg-red-500/10 border border-red-500/50 text-red-400'}`}>
              {success || error}
            </div>
          )}
